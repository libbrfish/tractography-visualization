function write_scalarfield_to_nifti_cropped(scalarfield, bounds, ref_filename, nifti_filename)
    ...
    scalarfield_no_nan = fillmissing(scalarfield, 'constant', 0);
    entropy = imresize3(scalarfield_no_nan(:,:,:), target_size, 'nearest'); 
    ...
    V(x_idx, y_idx, z_idx) = entropy;
    count = nnz(V ~= 0 & ~isnan(V))
    ...
    fprintf('Scalar entropy image saved to %s with reference alignment.\n', nifti_filename);
end
